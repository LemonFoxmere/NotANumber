...
let init_data = new Promise((res, rej) => {
    ...
    let dbExists = true;
    const dbPromise = (0, idb_1.openDB)(dbName, 1, {
        upgrade(db) {
    ...
    let grep_data = (start_chunk, db) => {
        return new Promise(async (res1, rej1) => {
            for (let i = start_chunk; i < 200; i++) {
                ...
                cache_data.push(row);
                if (i === 199) {
                    document.getElementById("load-bar").classList.add("disable-hidden");
                    document.getElementById("warn-time").classList.add("disable-hidden");
                    res1();
                }
                ;
                document.getElementById("load-bar").value = String(i);
                update_load_status_msg(`Fetched ${(i + 1) * 200}/40000 chunks`, "loader-status");
            }
            ;
        });
    };
    update_load_status_msg("Contacting database...", "loader-status");
    setTimeout(() => {
        if (!dbExists) {
            let k = setTimeout(() => {
                document.getElementById("warn-time").classList.remove("disable-hidden");
            }, 3000);
            dbPromise.then(async (db) => {
                let cycle_loading_text = null;
                document.getElementById("load-bar").classList.remove("disable-hidden");
                await grep_data(0, db);
            }).then(() => {
                clearTimeout(k);
                update_load_status_msg(`Finalizing data...`, "loader-status");
                setTimeout(() => {
                    (0, loadanim_1.stop_spinning_anim)();
            ...
            dbPromise.then(async (db) => {
                update_load_status_msg("Reading data to cache...", "loader-status");
                document.getElementById("load-bar").classList.remove("disable-hidden");
                let cache_db_data = new Promise((res_1, rej_1) => {
                    for (let i = 0; i < 200; i++) {
...
document.addEventListener("DOMContentLoaded", () => {
    init_data.then(val => {
        ...
        setTimeout(() => {
            ...
            (0, eventHandlers_1.start_listening_input)();
        }, 700);
    }).catch(err => {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.start_listening_input = void 0;
const loadUI_1 = __webpack_require__(/*! ./loadUI */ "./src/loadUI.ts");
const idb_1 = __webpack_require__(/*! idb */ "./node_modules/idb/build/index.js");
const app_1 = __webpack_require__(/*! ./app */ "./src/app.ts");
let start_listening_input = () => {
    const dbName = "ganout-db";
    const input_box = document.getElementById("input-box");
    if (!input_box)
        return "DOM failed to load";
    const dbPromise = (0, idb_1.openDB)(dbName, 1, {
    ...
    dbPromise.then(db => {
        input_box.addEventListener("mousemove", async (e) => {
            try {
                await (0, loadUI_1.set_image)(app_1.latentoutput[Math.floor(e.offsetX / input_box.clientWidth * 200)][Math.floor(e.offsetY / input_box.clientHeight * 200)]);
            }
            catch (error) {
        ...
        input_box.addEventListener("touchmove", async (e) => {
            ...
            try {
                await (0, loadUI_1.set_image)(app_1.latentoutput[Math.floor((e.touches[0].clientX - input_box.getBoundingClientRect().x) / input_box.getBoundingClientRect().width * 200)][Math.floor((e.touches[0].clientY - input_box.getBoundingClientRect().y) / input_box.getBoundingClientRect().height * 200)]);
            }
            catch (error) {
                console.warn("chunk failed to load.");
            }
...
exports.start_listening_input = start_listening_input;
    ...
    e.addEventListener("mouseup", () => {
        ...
        (0, animejs_1.default)({
            ...
            complete: () => {
                ...
                document.getElementById("clicker-blocker").classList.add("disable-hidden");
            }
        });
    });
});
let summon_card = (card_id) => {
    let e = document.getElementById(card_id);
    if (e)
        e.classList.remove("disable-hidden");
    document.getElementById("clicker-blocker").classList.remove("disable-hidden");
    (0, animejs_1.default)({
        targets: e,
        translateY: {
            value: "0rem",
...
let mobile_menu = {
    ...
    set: async function (shown, transition = true) {
        if (!transition) {
        ...
        if (shown) {
            ...
            document.getElementById("menu-open-btn").classList.remove("btn-menu-hidden");
...
document.getElementById("menu-open-btn").addEventListener("click", (e) => {
    mobile_menu.toggle();
});
document.getElementById("about-nav-btn").addEventListener("click", (e) => {
    summon_card('abt-card');
});
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.stop_spinning_anim = void 0;
const animejs_1 = __importDefault(__webpack_require__(/*! animejs */ "./node_modules/animejs/lib/anime.es.js"));
const animation_duration = 800;
const ring1_animation = (0, animejs_1.default)({
    targets: "#ring1",
    rotateX: 360,
    rotateY: 360,
...
const ring2_animation = (0, animejs_1.default)({
    targets: "#ring2",
    rotateX: -360,
    rotateY: -360,
...
const ring3_animation = (0, animejs_1.default)({
    targets: "#ring3",
    rotateX: -360,
...
const check_anim = (0, animejs_1.default)({
...
exports.stop_spinning_anim = stop_spinning_anim;
